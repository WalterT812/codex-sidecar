...
await Promise.all([
  build({entryPoints:['src/mobile/server.ts'],outfile:'dist/mobile/server.js',bundle:true,platform:'node',target:'node22',format:'esm'}),
...
await cp('src/mobile/web','dist/mobile/web',{recursive:true});

await writeFile('dist/mobile/package.json',JSON.stringify({type:'module',private:true}));
