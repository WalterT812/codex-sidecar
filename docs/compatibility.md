...
Status checked on **5 September 2026**. Version **0.1.0-alpha.2** is an early Windows companion, not an official extension API.
...
| Runtime | Node.js 24.16.0; all 99 automated tests, type checking, and bundled build passed, including duplicate-start, immediate-reopen, and coordinator lifecycle regressions |
...
Alpha.1's first real desktop launch reached `Components attached` and `SIDECAR_READY=1`. A duplicate launch during startup was rejected, and the coordinator persisted after the desktop exited. Alpha.2 adds a local instance handshake and tracks the original desktop PID plus exact creation time. The corrected restart workflow, native placement, and multi-window behavior still need interactive desktop acceptance. Static source inspection and browser tests are not a substitute for that check.
