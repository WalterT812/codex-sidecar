...
Selected task text overrides descendant native text tokens as well as inherited color. The selected row's icon is a transparent white outline. Notes, bookmarks and translation now open concurrently, with separate drafts and geometry. The duplicate tab row is hidden. Drag a panel header to move it; all eight edges/corners resize it. Header arrow keys move, bottom-right handle arrow keys resize, and Shift increases the step. Double-click a header or use Reset to restore its default position. Each tool has a layout scoped by the current conversation ID and window. Visibility is remembered per conversation in session storage (up to 100 conversation entries); geometry is also saved locally. The previous unscoped frame is migrated for the initially visible conversation. Panels stay open until explicitly closed; the pin control and pointer-leave auto-close behavior were removed. Conversation changes hide panels without recording a manual close, and returning restores their previous open state and geometry. Notes and translation content remain in the existing shared stores. User-positioned panels remain where placed instead of automatically avoiding the summary.
