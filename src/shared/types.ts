...
export interface Settings { locale: 'zh-CN' | 'en'; enabled: { quota: boolean; notes: boolean; bookmarks: boolean; artwork?: boolean; theme?: boolean; motion?: boolean; translation?: boolean; workspaces?: boolean }; panelPinned: boolean }
...
export interface StoredState { version: 1; revision: number; settings: Settings; notes: Note[]; bookmarks: Bookmark[]; translations?:TranslationRecord[] }
...
export type Action = 'ui.ready' | 'note.save' | 'note.delete' | 'bookmark.save' | 'bookmark.delete' | 'settings.patch' | 'quota.refresh' | 'open.link' | 'ui.detach' | 'translate' | 'translation.clear';
export interface BridgeRequest { id: string; action: Action; payload: Record<string, unknown> }
export type HostMessage = { type: 'snapshot'; state: StoredState; quota: QuotaSnapshot } | { type: 'result'; id: string; ok: boolean; error?: string; translation?: string };
